            ...
            for filename in os.listdir(table_path):
                if filename.endswith('.pickle') or filename.endswith('.pkl'):
                    ...
                    try:
                        ...
                        self.logger.warning(f"无法加载文件 {filename}: {str(e)}")
                        continue
        return result
    ...
    def save_data(self, df, table, if_exists='append', schema=None):
        ...
        try:
            ...
            if table_exists:
                ...
                self.logger.info(f"数据成功创建表 {schema}.{table}" if schema else f"数据成功创建表 {table}")
        ...
        try:
            ...
            if time_column:
                if start_date or end_date:
                    ...
                    self.logger.warning("警告：已指定时间列但未设置时间范围，将返回全部数据")
            ...
            self.logger.error("警告：已指定时间列但未设置时间范围，将返回全部数据")
