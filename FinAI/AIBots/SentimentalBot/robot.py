...
class SentimentalBot(BaseAI):
    ...
    def get_caches(self) -> pd.DataFrame:
        """获取缓存数据"""
        try:
            self.table_assistant = "aicache_assistant2"
            ...
            self.raw_assistant = pd.DataFrame(columns=["date", "category", "sub_category", "title", "summary", "opinion", "short_forecast", "long_forecast", "confidence"])
        try:
            ...
            self.logger.warning(f"[researcher] 读取缓存失败: {e}, 将使用空数据")
            self.raw_researcher = {}
    ...
    def save_cache(self, data: Union[pd.DataFrame, Dict], table: str, keys: Optional[list]=None) -> None:
        ...
        if isinstance(data, pd.DataFrame):  
            ...
            self.handler.save_data(
                ...
                table=table,
                keys=keys,
                schema=self.pydantic_model_to_arrow_schema(SugarSentimentalAssistantSchema)
            )
        elif isinstance(data, Dict):
    ...
    def assistant(self, data: dict, run_parallel: bool=False) -> List:
        ...
        if run_parallel:
            ...
            hit_cache, result = map(list, zip(*result_zip))
            self.logger.info(f"[Assistant] 并行处理完成: 命中缓存: {len([i for i in hit_cache if i is not False])} / {article_nums}")
        else:
            # 串行处理
            ...
            for i, article in enumerate(data):
                ...
                try:
                    ...
                    self.logger.warning(f"[Assistant] {article['title']} ai 分析失败: {e}")
                t1 = datetime.now()
                if hit_cache is True:
                    self.logger.info(f"[Assistant] 命中缓存: ({i+1} / {article_nums}), 标题: {article['title']}")
                else:
                    self.logger.info(f"[Assistant] AI分析: ({i+1} / {article_nums}), 标题: {article['title']}, 耗时: {t1 - t0}")
                time.sleep(20) 
        
        # 存储缓存
        normalized_df = self.normalize(pd.DataFrame(result))
        self.save_cache(
            data=normalized_df,
            table=self.table_assistant,
            keys=["date", "category", "sub_category", "title"]
        )
        self.logger.info(f"[Assistant] 缓存分析结果: {self.table_assistant}, 缓存大小: {len(result)}") 
        return result
    ...
    def researcher(self, input_reports: list) -> str:
        ...
        def _analyzing_assistant_reports(input_reports: list, cache: bool) -> Tuple[bool, Optional[Dict]]:
            ...
            # AI分析
            sys_prompt = self.read_md(os.path.join(self.filepath_prompt, "researcher.md"))
            try:
                answer = self.ai_api(user_prompt=user_prompt, sys_prompt=sys_prompt, json_output=True)
            except Exception as e:
                self.logger.warning(f"[researcher] AI 分析失败: {e}")
                answer = None
        ...
        # AI分析
        hit_cache, report = _analyzing_assistant_reports(input_reports)
        if hit_cache is True:
    ...
    def analyzing(self):
        ...
        assistant_reports = self.assistant(data, run_parallel=False)
        ...
        try:
            ...
            self.logger.info(f"{self.end_date} 研究报告, 邮件发送成功!")
        except Exception as e:
            self.logger.warning(f"{self.end_date} 研究报告, 邮件发送失败: {e}")
